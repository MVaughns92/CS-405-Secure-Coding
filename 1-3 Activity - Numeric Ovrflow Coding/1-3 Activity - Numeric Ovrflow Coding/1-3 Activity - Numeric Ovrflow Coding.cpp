...
T add_numbers(T const& start, T const& increment, unsigned long int const& steps)
{
    ...
    {
        // Detect overflow by checking numeric limits
        if (result >= 0 && (increment > (std::numeric_limits<T>::max() - result)))
        {
            // Throw error for overflow
            throw std::overflow_error("Overflow Detected! Numerical limits of " + std::to_string(std::numeric_limits<T>::max()) + "have been exceeded!\n");
        }
