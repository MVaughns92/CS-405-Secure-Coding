...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
    ...
    // Set regex pattern to check for "OR value=value;" style SQL injection attack
    std::regex re("('(.*)[a-zA-Z](.*)'='(.*)[a-zA-Z](.*)')|([0-9]=[0-9])");
    std::smatch match;

    // Check if SQL query matches regex pattern for specific type of injection
    if (std::regex_search(sql.begin(), sql.end(), match, re) && match.size() > 1) {
        // If match found, display warning message, then return false and exit method
